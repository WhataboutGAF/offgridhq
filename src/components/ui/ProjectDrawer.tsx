...
export default function ProjectDrawer({ project, onClose }: ProjectDrawerProps) {
  ...
  // Mock technical data (In a real app, this would be in projects.json)
  const techStack = ["Next.js", "Tailwind", "Framer", "AI Studio"];
  const buildDate = "Q1 2026";
  const uptime = "99.9%";
          ...
          >
            ...
            <div className="p-6 border-b-[2.5px] border-foreground/10 flex justify-between items-center bg-muted/20">
               <div className="flex flex-col">
                  <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">SERVICE MANUAL</p>
                  <p className="text-[14px] font-black uppercase">NODE_{project.id.toUpperCase()}</p>
               </div>
               <button 
                  onClick={onClose}
            ...
            <div className="p-8 space-y-10">
               ...
               <div className="space-y-4">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase text-foreground/40">
                     <Binary className="w-3 h-3" /> TECHNICAL DISPATCH
                  </div>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="p-6 bg-secondary/10 border-[2.5px] border-secondary rounded-2xl flex flex-col gap-1 shadow-[4px_4px_0_0_theme(colors.secondary.DEFAULT)] transition-transform hover:-translate-y-1">
                     <div className="flex items-center gap-2 text-[10px] font-black text-secondary-foreground uppercase">
                        <Calendar className="w-3 h-3" /> ORIGIN DATE
                     </div>
                  ...
                  <div className="p-6 bg-primary/10 border-[2.5px] border-primary rounded-2xl flex flex-col gap-1 shadow-[4px_4px_0_0_theme(colors.primary.DEFAULT)] transition-transform hover:-translate-y-1">
                     <div className="flex items-center gap-2 text-[10px] font-black text-primary-foreground uppercase">
                        <Activity className="w-3 h-3" /> UPTIME STATUS
                     </div>
               ...
               {/* Tech Array */}
               <div className="space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-foreground/40 px-1">INTEGRATED STACK</p>
               ...
               <div className="pt-8 border-t-[2.5px] border-foreground/10 flex flex-col gap-4">
                  <button className="w-full bg-foreground text-background py-5 rounded-2xl font-black uppercase tracking-[0.2em] transform transition-all active:scale-[0.98] active:translate-y-1 shadow-[6px_6px_0_0_rgba(0,0,0,0.2)] flex items-center justify-center gap-3 group">
                     INITIATE UPLINK <ExternalLink className="w-5 h-5 group-hover:scale-125 transition-transform" />
                  </button>
                  <p className="text-[9px] text-center font-black text-muted-foreground uppercase tracking-widest opacity-40">End of Technical Document — OFFGRID NODE {project.id.toUpperCase()}</p>
               </div>
